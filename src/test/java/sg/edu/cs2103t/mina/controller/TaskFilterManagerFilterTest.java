...
public class TaskFilterManagerFilterTest {
	...
	public void testFilterCombination(){
		...
		FilterType[] filters = {FilterType.DEADLINE,
														FilterType.EVENT};
		
		ArrayList<Task<?>> test = getResult(filters);
		int actualSize = tdmStub.getAllDeadlineTasks().size() + 
										 tdmStub.getAllEventTasks().size();
		
		assertTrue("Contains only uncompleted deadline and events!",
								actualSize==test.size() &&
								checkTwoTaskTypes(test, 
																	TaskType.DEADLINE, 
																	TaskType.EVENT,
																	UNCOMPLETE));	
		
		//With deadlines and todos
		filters = new FilterType[]{FilterType.DEADLINE,
															 FilterType.TODO};
		test = getResult(filters);
		actualSize = tdmStub.getAllDeadlineTasks().size() + 
								 tdmStub.getAllTodoTasks().size();
		assertTrue("Contains only uncompleted deadline and todos!",
								actualSize==test.size() &&
								checkTwoTaskTypes(test, 
																	TaskType.DEADLINE, 
																	TaskType.TODO, 
		...
		filters = new FilterType[]{FilterType.EVENT,
				 											 FilterType.TODO};
		test = getResult(filters);
		actualSize = tdmStub.getAllEventTasks().size() + 
								 tdmStub.getAllTodoTasks().size();
		assertTrue("Contains only uncompleted todos and events!",
								actualSize==test.size() &&
								checkTwoTaskTypes(test, 
																	TaskType.EVENT, 
																	TaskType.TODO,
		...
		filters = new FilterType[]{FilterType.COMPLETE,
				 											 ...
				 											 FilterType.DEADLINE};
		test = getResult(filters);
		actualSize = tdmStub.getPastEventTasks().size() + 
				 				 tdmStub.getPastDeadlineTasks().size(); 
		assertTrue("Contains completed deadlines and events!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.DEADLINE,
																	 TaskType.EVENT, COMPLETE));
		...
		filters = new FilterType[]{FilterType.COMPLETE,
						 									 ...
						 									 FilterType.DEADLINE};
		test = getResult(filters);
		actualSize = tdmStub.getPastTodoTasks().size() + 
				 				 tdmStub.getPastDeadlineTasks().size(); 
		assertTrue("Contains completed deadlines and todos!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.DEADLINE,
		...
		assertTrue("Contains completed events and todos!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.EVENT,
																	 TaskType.TODO, COMPLETE));
		...
		actualSize = tdmStub.getPastTodoTasks().size() + 
				 				 tdmStub.getPastEventTasks().size() +
				 				 tdmStub.getPastDeadlineTasks().size(); 
		assertTrue("Contains all completed tasks",
								test.size() == actualSize &&
										hasCompletedTasks(test, getCompletedDummyTasks()));		
		...
		actualSize = tdmStub.getPastTodoTasks().size() + 
				 				 ...
				 				 tdmStub.getAllEventTasks().size() + 
								 tdmStub.getAllTodoTasks().size();
		assertTrue("Contains all event and todos!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.EVENT,
																	 TaskType.TODO,
																	 EVERYTHING));
		...
		filters = new FilterType[]{FilterType.COMPLETE_PLUS,
															 ...
															 FilterType.EVENT};
		test = getResult(filters);
		actualSize = tdmStub.getPastDeadlineTasks().size() + 
				 				 tdmStub.getPastEventTasks().size() + 
				 				 tdmStub.getAllEventTasks().size() + 
								 tdmStub.getAllDeadlineTasks().size();
		assertTrue("Contains all event and deadlines!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.EVENT,
																	 TaskType.DEADLINE,
																	 EVERYTHING));
		...
		test = getResult(filters);
		actualSize = tdmStub.getPastDeadlineTasks().size() + 
				 				 tdmStub.getPastTodoTasks().size() + 
				 				 tdmStub.getAllTodoTasks().size() + 
								 tdmStub.getAllDeadlineTasks().size();
		assertTrue("Contains all deadlines and todos!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.TODO,
																	 TaskType.DEADLINE,
																	 EVERYTHING));
		...
		filters = new FilterType[]{FilterType.PRIORITY,
				 											 FilterType.DEADLINE};
		test = getResult(filters);
		Iterator<DeadlineTask> deadlineSet = tdmStub.getAllDeadlineTasks().iterator();
		...
		filters = new FilterType[]{FilterType.PRIORITY,
															 ...
															 FilterType.TODO};
		test = getResult(filters);
		actualSize = tdmStub.getAllTodoTasks().size() + 
				 				 tdmStub.getAllDeadlineTasks().size(); 
		assertTrue("Contains deadlines and todos!",
								test.size() == actualSize &&
								checkTwoTaskTypes(test, TaskType.DEADLINE,
													 				TaskType.TODO, UNCOMPLETE));
	...
	private boolean isValidInTaskCombi(Task<?> task, TaskType firstType,
		...
		if(taskStatus==EVERYTHING){
			...
			return isCorrectStatus(task.isCompleted(), taskStatus) && 
						 isCorrectType;
		}
	}

	private boolean isCorrectStatus(boolean isCompleted, int taskStatus) {
		return (isCompleted && taskStatus==COMPLETE) ||
					 (!isCompleted && taskStatus==UNCOMPLETE);
	}
