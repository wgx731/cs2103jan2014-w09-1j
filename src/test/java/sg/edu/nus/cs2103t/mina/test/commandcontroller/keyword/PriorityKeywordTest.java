...
public class PriorityKeywordTest {
    ...
    public static void setUpBeforeClass() throws Exception {
        logger = LogManager.getLogger(PriorityKeywordTest.class.getName());
    ...
    public void setUp(){
        ...
        String[] suffixTokensNoDelimitTokensArr = suffixNoDelimitControl.split(" ");
    ...
    public void testPrefix() throws ParseException{
        logger.info("Testing priority prefix");
        prefixTokens = priority.processKeyword(prefixTokens, 1, argument);
        
        assertEquals("L", argument.getKeywordValue(SimpleKeyword.PRIORITY).trim());
        
        logger.info("Testing to ensure that all entries are null");
        assertEquals(allNull, prefixTokens);
    }
    
    @Test
    public void testSuffix() throws ParseException{
        logger.info("Testing priority suffix");
        suffixTokens = priority.processKeyword(suffixTokens, 0, argument);
        
        assertEquals("H", argument.getKeywordValue(SimpleKeyword.PRIORITY).trim());
    ...
    public void testSuffixNoDelmit() throws ParseException{
        logger.info("Testing priority suffix but with no delimiter");
        suffixTokensNoDelimitTokens = priority.processKeyword(suffixTokensNoDelimitTokens, 0, argument);
        
        assertEquals("M", argument.getKeywordValue(SimpleKeyword.PRIORITY).trim());
